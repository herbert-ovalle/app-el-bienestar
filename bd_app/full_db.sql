...
-- Schema dbAppMovilClie
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `dbAppMovilClie` DEFAULT CHARACTER SET armscii8 ;
USE `dbAppMovilClie` ;
...
-- -----------------------------------------------------
-- Placeholder table for view `vstUsuarios`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `vstUsuarios` (`idUsuarios` INT, `usuario` INT, `idEstados` INT, `idPuesto` INT, `'nombrePuesto'` INT, `idAgencia` INT, `'nombreAgencia'` INT, `estado` INT, `idPerfil` INT, `nombrePerfil` INT, `descripcion` INT, `modulos` INT, `aplicaciones` INT, `idDatosPersonales` INT, `nombres` INT, `telefono` INT, `direccion` INT, `correoElectronico` INT, `fotografia` INT, `fechaRegistro` INT);
...
-- View `vstUsuarios`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `vstUsuarios`;
DROP VIEW IF EXISTS `vstUsuarios` ;
USE `dbAppMovilClie`;
...
SELECT 
    ...
    per.nombrePerfil,
    per.descripcion,
    JSON_EXTRACT(pm.permisosPerfil, '$.modulos') AS modulos,
    CASE 
        WHEN us.idPerfil = 1 THEN JSON_EXTRACT(pm.permisosPerfil, '$.aplicaciones')
		ELSE (SELECT 
				JSON_ARRAYAGG(permisosApp) AS permisos_json
			FROM
				permisosAplicacion
			WHERE
				idPerfil = us.idPerfil AND permisosApp IS NOT NULL)
    END AS aplicaciones,
	dp.*
FROM
	usuarios AS us
	JOIN datosPersonales dp
		ON us.idDatosPersonales = dp.idDatosPersonales
	JOIN estados AS es
		ON us.idEstados = es.idEstados
	JOIN perfiles AS per
		ON us.idPerfil = per.idPerfil
	JOIN permisosIntranet AS pm
		ON per.idPerfil = pm.idPerfil
    JOIN usuarios.puesto AS pst
		ON pst.id_puesto = us.idPuesto
	JOIN usuarios.agencia AS ag
		ON ag.id_agencia = us.idAgencia;
