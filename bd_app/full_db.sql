...
CREATE TABLE IF NOT EXISTS `log_usuarios` (
  ...
  `idReferencia` CHAR(30) NOT NULL COMMENT 'Referencia de la tabla en la que se realizo el login',
  `refTabla` CHAR(50) NOT NULL,
  `refAccion` CHAR(50) NOT NULL,
  `logUsuario` JSON NOT NULL,
...
CREATE TABLE IF NOT EXISTS `asociado` (
  ...
  `telefono` CHAR(15) NULL DEFAULT '',
  `correoElectronico` CHAR(75) NULL DEFAULT '',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;

CREATE INDEX `ix_dpi` ON `asociado` (`dpi` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `actualizacionDatos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `actualizacionDatos` ;

CREATE TABLE IF NOT EXISTS `actualizacionDatos` (
  `idActualizacion` INT NOT NULL AUTO_INCREMENT,
  `fechaActualizacion` DATE NOT NULL,
  `horaActualizacion` TIME NOT NULL,
  `usuarioActualizacion` CHAR(20) NULL DEFAULT '',
...
-- Table `captaciones`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `captaciones` ;

CREATE TABLE IF NOT EXISTS `captaciones` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `dpi` CHAR(15) NOT NULL,
  `producto` CHAR(100) NOT NULL DEFAULT '',
  `tipoProducto` CHAR(100) NOT NULL DEFAULT '',
  `numeroCuenta` CHAR(20) NOT NULL DEFAULT '',
  `montoDisponible` DOUBLE NOT NULL DEFAULT 0,
  `idActualizacion` INT NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_captaciones_actualizacionDatos1`
    FOREIGN KEY (`idActualizacion`)
    REFERENCES `actualizacionDatos` (`idActualizacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_captaciones_actualizacionDatos1_idx` ON `captaciones` (`idActualizacion` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `colocaciones`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `colocaciones` ;
...
CREATE TABLE IF NOT EXISTS `colocaciones` (
  ...
  `idActualizacion` INT NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_colocaciones_actualizacionDatos1`
    FOREIGN KEY (`idActualizacion`)
    REFERENCES `actualizacionDatos` (`idActualizacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `fk_colocaciones_actualizacionDatos1_idx` ON `colocaciones` (`idActualizacion` ASC) VISIBLE;
...
-- -----------------------------------------------------
-- procedure prCargaCatalogos
-- -----------------------------------------------------

USE `dbAppMovilClie`;
DROP procedure IF EXISTS `prCargaCatalogos`;

DELIMITER $$
USE `dbAppMovilClie`$$
CREATE PROCEDURE prCargaCatalogos()
BEGIN
    DECLARE _idActualizacion INT DEFAULT 0;
    ...
    /*-- Verificar si hay una transacción activa y comenzar una nueva si no la hay*/
    IF (SELECT @@autocommit) = 1 THEN
        START TRANSACTION;
    END IF;

    /*-- INSERTAR FECHA/HORA DE ACTUALIZACIÓN */
    INSERT INTO actualizacionDatos(
        fechaActualizacion, 
        horaActualizacion, 
        usuarioActualizacion
    ) VALUES (
        CURDATE() - INTERVAL 1 DAY,  /*-- Equivalente a DATE_SUB(DATE(NOW()), INTERVAL 1 DAY)*/
        CURTIME(),                   /*-- Hora actual sin fecha*/
        NULL
    );
    /*-- OBTENER EL ULTIMO ID INGRESADO*/
    SET _idActualizacion = LAST_INSERT_ID();
    
    -- Borrado en bloques para cada tabla
    REPEAT
        DELETE FROM asociado LIMIT 1000;
    UNTIL ROW_COUNT() = 0 END REPEAT;

    REPEAT
        DELETE FROM captaciones LIMIT 1000;
    UNTIL ROW_COUNT() = 0 END REPEAT;

    REPEAT
        DELETE FROM colocaciones LIMIT 1000;
    UNTIL ROW_COUNT() = 0 END REPEAT;

    -- Reinicio de IDs (¡OJO con el COMMIT automático!)
    ALTER TABLE asociado AUTO_INCREMENT = 1;
    ALTER TABLE captaciones AUTO_INCREMENT = 1;
    ALTER TABLE colocaciones AUTO_INCREMENT = 1;

    /*-- INSERTAR DATOS CON TRANSACCIÓN */
    /*-- 1. ASOCIADOS*/
    INSERT INTO asociado(dpi, nombres, telefono, correoElectronico)
    SELECT
        Dpi,
        CONCAT_WS(' ', Nombre1, Nombre2, Nombre3, Apellido1, Apellido2),
        celular,
        CorreoElectronico
    FROM
        asociado_t24.comndatosclientes 
    WHERE 
        CodigoCliente > 155 
        AND Dpi != 0 
        AND LENGTH(Dpi) = 13 
        AND TipoCliente = 'INDIVIDUAL';

    /*-- 2. CAPTACIONES*/
    INSERT INTO captaciones(dpi, producto, tipoProducto, numeroCuenta, montoDisponible, idActualizacion)
    SELECT
        cCli.Dpi,
        cMC.categoriaProducto,
        cMC.grupo,
        cMC.NumeroCuenta,
        cMC.MontoDisponible,
        _idActualizacion
    FROM
        asociado_t24.comndatosclientes AS cCli
    LEFT JOIN
        asociado_t24.ccomndatoscaptaciones AS cMC 
        ON cCli.CodigoCliente = cMC.Cliente
    WHERE 
        cMC.categoriaProducto IS NOT NULL
        AND cMC.grupo IS NOT NULL
        AND cCli.CodigoCliente > 155 
        AND cCli.Dpi != 0 
        AND LENGTH(cCli.Dpi) = 13 
        AND cCli.TipoCliente = 'INDIVIDUAL';

    /**-- 3. COLOCACIONES*/
    INSERT INTO colocaciones(dpi, producto, tipoProducto, numeroCuenta, montoDisponible, idActualizacion)
    SELECT
        cCli.Dpi,
        colo.categoriaProducto,
        colo.grupo,
        colo.NUMERODOCUMENTO,
        colo.SALDOCAPITAL,
        _idActualizacion
    FROM
        asociado_t24.comndatosclientes AS cCli
    JOIN
        asociado_t24.ccomndatoscolocaciones AS colo 
        ON cCli.CodigoCliente = colo.Cliente
    WHERE 
        colo.categoriaProducto IS NOT NULL
        AND colo.grupo IS NOT NULL
        AND cCli.CodigoCliente > 155 
        AND cCli.Dpi != 0 
        AND LENGTH(cCli.Dpi) = 13 
        AND cCli.TipoCliente = 'INDIVIDUAL';

    COMMIT; /**-- Confirma todos los cambios si todo está correcto*/

    SELECT 'success' AS 'respuesta', 'Catálogos agregados con éxito' AS 'mensaje';
...
USE `dbAppMovilClie`$$
DROP TRIGGER IF EXISTS `datosPersonales_AFTER_UPDATE` $$
USE `dbAppMovilClie`$$
CREATE DEFINER = CURRENT_USER TRIGGER `dbAppMovilClie`.`datosPersonales_AFTER_UPDATE` AFTER UPDATE ON `datosPersonales` FOR EACH ROW
BEGIN
    ...
    IF OLD.nombres <> NEW.nombres THEN
        SET @_log = JSON_SET( @_log, '$.nombres', OLD.nombres );
    END IF;
    
    # SI CAMBIO EL TELEFONO
    IF OLD.telefono <> NEW.telefono THEN
        SET @_log = JSON_SET( @_log, '$.telefono', OLD.telefono );
    END IF;
    
    # SI CAMBIO LA DIRECCION
    IF OLD.direccion <> NEW.direccion THEN
        SET @_log = JSON_SET( @_log, '$.direccion', OLD.direccion );
    END IF;
    
    # SI CAMBIO EL CORREO
    IF OLD.correoElectronico <> NEW.correoElectronico THEN
        SET @_log = JSON_SET( @_log, '$.correoElectronico', OLD.correoElectronico );
    END IF;
    
    # SI CAMBIO LA FOTOGRAFIA
    IF OLD.fotografia <> NEW.fotografia THEN
        SET @_log = JSON_SET( @_log, '$.fotografia', OLD.fotografia );
    END IF;

    # SI HUBO CAMBIO DE LOG
    IF NOT( @_log <=> '{}' ) THEN
        INSERT INTO log_usuarios( idlog_usuario, idReferencia, refTabla, refAccion, logUsuario, fechaRegistro )
        VALUES( NULL, OLD.idDatosPersonales, 'tblDatosPersonales', 'update', @_log, NOW() );
    END IF;
...
USE `dbAppMovilClie`$$
DROP TRIGGER IF EXISTS `datosPersonales_AFTER_DELETE` $$
USE `dbAppMovilClie`$$
CREATE DEFINER = CURRENT_USER TRIGGER `dbAppMovilClie`.`datosPersonales_AFTER_DELETE` AFTER DELETE ON `datosPersonales` FOR EACH ROW
BEGIN

	-- Registra todos los valores de OLD directamente
    SET @_log = JSON_OBJECT(
        'nombres', OLD.nombres,
        'telefono', OLD.telefono,
        'direccion', OLD.direccion,
        'correoElectronico', OLD.correoElectronico,
        'fotografia', OLD.fotografia
    );

    -- Inserta el log (siempre que haya datos)
    INSERT INTO log_usuarios( idlog_usuario, idReferencia, refTabla, refAccion, logUsuario, fechaRegistro )
    VALUES( NULL, OLD.idDatosPersonales, 'tblUsuarios', 'delete', @_log, NOW() );
...
USE `dbAppMovilClie`$$
CREATE DEFINER = CURRENT_USER TRIGGER `dbAppMovilClie`.`usuarios_AFTER_UPDATE` AFTER UPDATE ON `usuarios` FOR EACH ROW
BEGIN
    ...
    # SI CAMBIO EL DPI
    IF OLD.dpi <> NEW.dpi THEN
        SET @_log = JSON_SET( @_log, '$.dpi', OLD.dpi );
    END IF;
    
    # SI CAMBIO EL ID ESTADO
    IF OLD.idEstados <> NEW.idEstados THEN
        SET @_log = JSON_SET( @_log, '$.idEstados', OLD.idEstados );
    END IF;
    
    # SI CAMBIO EL ID PERFIL
    IF OLD.idPerfil <> NEW.idPerfil THEN
        SET @_log = JSON_SET( @_log, '$.idPerfil', OLD.idPerfil );
    END IF;

    # SI HUBO CAMBIO DE LOG
    IF NOT( @_log <=> '{}' ) THEN
        INSERT INTO log_usuarios( idlog_usuario, idReferencia, refTabla, refAccion, logUsuario, fechaRegistro )
        VALUES( NULL, OLD.idUsuario, 'tblUsuarios', 'update', @_log, NOW() );
    END IF;
...
USE `dbAppMovilClie`$$
DROP TRIGGER IF EXISTS `usuarios_AFTER_DELETE` $$
USE `dbAppMovilClie`$$
CREATE DEFINER = CURRENT_USER TRIGGER `dbAppMovilClie`.`usuarios_AFTER_DELETE` AFTER DELETE ON `usuarios` FOR EACH ROW
BEGIN

	-- Crea el log con los valores de OLD
    SET @_log = JSON_OBJECT( 'dpi', OLD.dpi, 'idEstados', OLD.idEstados, 'idPerfil', OLD.idPerfil );

    -- Inserta el log (registra todos los campos eliminados)
    INSERT INTO log_usuarios( idlog_usuario, idReferencia, refTabla, refAccion, logUsuario, fechaRegistro )
    VALUES( NULL, OLD.idUsuario, 'tblUsuarios', 'delete', @_log, NOW() );
